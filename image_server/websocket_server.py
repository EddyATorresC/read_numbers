...
async def hello(websocket, path):
    ...
    async def prediction_or_training(values):
        async with websockets.connect(
                'ws://192.168.100.246:9002') as websocket:
            #Código de cálculo para predección o entrenamiento
            #Variables de inicio
            #print(values[1])
            ...
            if(prediction[0][0][0]>prediction[0][0][1]):
                ...
                return_value = 1
            await websocket.send(str(return_value))

    await prediction_or_training(values[1])


start_server = websockets.serve(hello, '192.168.100.246', 9003)
